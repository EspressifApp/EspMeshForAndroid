    ...
    var ColorPicker = v.extend({
        ...
        methods:{
            show: function() {
                ...
                console.log(self.isAss)
                if (self.isAss && self.isAss == "true") {
                    self.initColorAss(hsbColor);
                } else {
            ...
            initColor: function (hsbColor) {
                var doc = $("#picker-wrapper"),
                    width = doc.width(),
                    height = doc.height(),
                    totalH = $("#ipad").height();
                this.initSize = height * 0.55;
                if (this.initSize > 240) {
                    this.initSize = 240;
                }
                totalH = totalH - height;
                var leftWidth = width / 0.7 * 0.3;
                height = height * 0.7;
                console.log(height);
                this._initColorPicker(hsbColor, this.colorId, (width - this.initSize) / 2 + leftWidth,
                    (height - this.initSize) / 2 + totalH + 20, true);
            },
            initColorAss: function(hsbColor) {
                var doc = $("#picker-wrapper"),
                    width = doc.width(),
                    height = doc.height(),
                    totalH = $("#ipad").height(),
                    totalW = $("#ipad").width();
                this.initSize = totalH *0.7 * 0.55;
                console.log(totalH);
                console.log(this.initSize);
                if (this.initSize > 240) {
                    this.initSize = 240;
                }
                totalH = (totalH - height) / 2;
                this._initColorPicker(hsbColor, this.colorId, (totalW - this.initSize) / 2,
                    (height - this.initSize) / 2 + totalH - 60, true);
            },
            showPicker: function () {
